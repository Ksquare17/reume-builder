    ...
    try {
        ...
        if (!user) {
            ...
            user = new User({
                ...
                avatar: req.body.avatar?req.body.avatar:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT-HmAlYRaMiTx6PqSGcL9ifkAFxWHVPvhiHQ&s"
            });
...
const emailPasswordSignIn = async (req, res) => {
    try {
        // Check if the request body contains necessary fields
        if (!req.body.email ) {
            return res.status(400).json({ message: "Missing required fields" });
        ...
        // Log the error for debugging
        console.error("Error in email password sign in:", error);
        // Send appropriate error response
        return res.status(500).json({ success: false, error: "Internal Server Error" });
    }
}
...
module.exports = { googleSignIn,emailPasswordSignIn };